"""No-OCR Streamlit app for GeoExtract."""
...
# Import no-OCR processing service
try:
    from processing_service_no_ocr import NoOCRProcessingService
    print("✅ NoOCRProcessingService imported successfully")
...
# Title
st.markdown('<h1 class="main-header">🗺️ GeoExtract - No OCR Version</h1>', unsafe_allow_html=True)
st.markdown("### Simulated Geological Report Data Extraction System")
st.info("⚠️ This version uses simulated OCR and extraction to avoid Python version compatibility issues.")
...
ocr_engine = st.sidebar.selectbox(
    ...
    index=0,
    disabled=True,
    help="Using simulated OCR to avoid Python version issues"
)
        ...
        if st.button("🚀 Process Documents", type="primary"):
            with st.spinner("Processing documents..."):
                ...
                with tempfile.TemporaryDirectory() as temp_dir:
                    ...
                    # Process files using no-OCR processing service
                    progress_bar = st.progress(0)
                    status_text = st.empty()
                    
                    # Initialize no-OCR processing service
                    processing_service = NoOCRProcessingService(
                    ...
                    # Download results using no-OCR export
                    if all_documents:
                        # Export data
...
with tab2:
    ...
    with col1:
        ...
        # Add sample markers (simulated)
        sample_locations = [
            {"lat": 37.7749, "lon": -122.4194, "name": "Drill Hole DH-001", "type": "drill_hole"},
            {"lat": 37.7849, "lon": -122.4094, "name": "Sample Site S001", "type": "sample_site"},
            {"lat": 37.7649, "lon": -122.4294, "name": "Mine Site", "type": "mine"},
        ]
        ...
        for loc in sample_locations:
            ...
            if loc["type"] == "drill_hole":
                icon_color = "red"
                icon_name = "play"
            elif loc["type"] == "sample_site":
                icon_color = "green"
                icon_name = "star"
            elif loc["type"] == "mine":
                icon_color = "orange"
...
with tab3:
    ...
    # Sample data for analysis (simulated)
    sample_data = {
...
st.markdown("---")
st.markdown(
    "**GeoExtract - No OCR Version** - Simulated Geological Report Data Extraction System | "
    "Built with ❤️ using Python, Streamlit, and simulated processing"
)
